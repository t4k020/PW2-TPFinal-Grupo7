...
class PreguntaModel
{
    ...
    public function getPreguntasHechasPorUsuarioGrafico()
    {
        $hoy    = date('Y-m-d 00:00:00');
        $semana = date('Y-m-d H:i:s', strtotime('-7 days'));
        $mes    = date('Y-m-d H:i:s', strtotime('-30 days'));
        ...
        $sql = "SELECT 
        SUM(CASE WHEN fechaCreacion >= '{$hoy}' AND creadoPor != 'ADMIN' THEN 1 ELSE 0 END) as hoy,
        SUM(CASE WHEN fechaCreacion >= '{$semana}' AND creadoPor != 'ADMIN' THEN 1 ELSE 0 END) as semana,
        SUM(CASE WHEN fechaCreacion >= '{$mes}' AND creadoPor != 'ADMIN' THEN 1 ELSE 0 END) as mes,
        SUM(CASE WHEN fechaCreacion >= '{$anio}' AND creadoPor != 'ADMIN' THEN 1 ELSE 0 END) as anio,
        COUNT(CASE WHEN creadoPor != 'ADMIN' THEN 1 END) as todo
    FROM Pregunta";
